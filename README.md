...
`video` 从当前文章 Page Bundle 读取本地视频文件。`src` 必填，也可以使用位置参数；`poster`、`caption`、`preload`、`type`、`autoplay`、`loop`、`muted`、`max-width` 和 `max-height` 可选。`max-width`、`max-height` 接收 CSS 长度或百分比值，例如 `720px`、`80%`。该 shortcode 不需要正文内容，因此旧文章中的未自闭合写法也可以继续使用：
...
{{< video src="demo.mp4" max-width="720px" max-height="480px" >}}
```
